...
def criterion(input,target,loss_weight = None,num_classes:int=2,dice:bool=True,ignore_index:int=-100):
    losses={}
...
def train_one_epoch(model,optimizer,data_loader,device,epoch,num_classes,lr_scheduler,print_freq=10,scaler=None):
    ...
    if num_classes ==2:
    #Set the loss weights of background and foreground in cross_entropy
    #loss_weight = torch.tensor([0.5,0.5])
    ...
    for image,target in metric_logger.log_every(data_loader,print_freq,header):
        image,target = image.to(device),target.to(device)
        with torch.cuda.amp.autocast(enabled=scaler is None):
            output = model(image)
